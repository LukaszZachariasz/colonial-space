...
export const selectBuildingObjectsByProduction0AndNotBuilt = (): BuildingObjectState[] => selectBuildings().map((el: BuildingState) => el.scopes).flat().map((el: BuildingScopeState) => el.objects).flat().filter((el: BuildingObjectState) => el.productionLeft === 0 && el.isBuilt === false);
...
export const selectBuildingObjectsByType = (type: BuildingObjectType): BuildingObjectState[] => selectBuildings().map((el: BuildingState) => el.scopes).flat().map((el: BuildingScopeState) => el.objects).flat().filter((el: BuildingObjectState) => el.type === type);
