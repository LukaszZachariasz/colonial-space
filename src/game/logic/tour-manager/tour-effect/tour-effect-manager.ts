...
export class TourEffectManager {
    ...
    constructor() {
        this.executeEffectGroup$.pipe(
            ...
            tap(() => store().tour.tourEffects
                .filter((el: TourEffect) => el.toTour)
                .filter((el: TourEffect) => store().tour.tour >= el.toTour)
    ...
    public addTourEffect(effect: TourEffect): void {
        store().tour.tourEffects.push(effect);
        store().tour.tourEffects = store().tour.tourEffects.sort(this.sortByPriority);
    }

    public removeTourEffect(effect: TourEffect): void {
        store().tour.tourEffects = store().tour.tourEffects.filter((tourEffect: TourEffect) => tourEffect !== effect);
    }
    ...
    private copyEffectsExecutedInThisTour(): TourEffect[] {
        return [...store().tour.tourEffects]
            .filter((el: TourEffect) => el.fromTour === undefined || el.fromTour <= store().tour.tour + 1 && (el.toTour === undefined || el.toTour > store().tour.tour));
    }
